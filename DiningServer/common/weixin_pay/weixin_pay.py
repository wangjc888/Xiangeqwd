...
class WeiXinPay(object):
	...
	def post_xml_ssl(self):
		...
		response = post_xml_ssl(self.url,
								data = xml, 
...
class Refund(WeiXinPay):  
	...
	def post(self, out_trade_no, out_refund_no, total_fee, refund_fee, op_user_id, **kwargs):
		tmp_kwargs = {
					  "out_trade_no": out_trade_no,
...
class RefundQuery(WeiXinPay):  
	...
	def __init__(self, appid, mch_id, api_key):
		super(RefundQuery, self).__init__(appid, mch_id, api_key)
...
class WechatPush(object):
	...
	def postMsg(self,url,data):
		return requests.post(url,data)

	def pushMsg(self,touser,template_id,url,data):         
		dictMsg = {'touser': touser, 'template_id':template_id, 'url':url, 'data':data}
		jsonMsg = json.dumps(dictMsg)
		AccessToken = self.getAccessToken()
		requstUrl = 'https://api.weixin.qq.com/cgi-bin/message/template/send?access_token='+AccessToken
		response = self.postMsg(requstUrl,jsonMsg)
