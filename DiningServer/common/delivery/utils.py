...
'''使用证书'''
def post_xml_ssl(url, xml, cert=True, post=True):
    return requests.post(url, data=xml, verify=False, cert=("/path/to/DiningHouse/DiningServer/ssl/apiclient_cert.pem" "/path/to/DiningHouse/DiningServer/ssl/apiclient_key.pem"))
# 其中SSLCERT_PATH, SSLKEY_PATH 为你的crt key(pem)文件在你服务器上的相对路径。s